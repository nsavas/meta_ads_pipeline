...
- One call per account, not one call per ad. GET /{ad_account_id}/insights
  with level=ad returns ad-level rows for *every* ad in the account in one
  paginated call -- confirmed against Meta's Marketing API docs on
  2026-08-19. Unlike Pinterest, there's no "list every ad's ID first, then
  batch analytics calls" step needed.
