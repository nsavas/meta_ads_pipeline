...
same /{ad_account_id}/insights endpoint and level=ad, with a `breakdowns`
parameter added. Produces a different grain of table: one row per
(ad, date, age, gender) instead of one row per (ad, date).
...
def main():
    ...
    for ad_account_id in ad_account_ids:
        insights = fetch_insights(ad_account_id, LEVEL, start_date, end_date, access_token,
                                   API_FIELDS, breakdowns=BREAKDOWNS)
