...
- One call per account, not one call per ad set. GET /{ad_account_id}/insights
  with level=adset returns ad-set-level rows for *every* ad set in the
  account in one paginated call -- see meta_ads_to_iceberg_glue_job.py's
