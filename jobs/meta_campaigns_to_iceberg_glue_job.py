...
- One call per account, not one call per campaign. GET /{ad_account_id}/insights
  with level=campaign returns campaign-level rows for *every* campaign in
  the account in one paginated call -- see meta_ads_to_iceberg_glue_job.py's
  docstring for the full explanation of why this differs from Pinterest's
  list-then-batch pattern.
- `objective` is included as dimensional context (Meta's insights rows do
  ...
  meaningfully snapshot. See meta_dimensions_to_iceberg_glue_job.py for
...
from meta_analytics import fetch_insights
...
logger = logging.getLogger("meta_campaigns_to_iceberg")
...
def main():
    ...
    for ad_account_id in ad_account_ids:
        insights = fetch_insights(ad_account_id, LEVEL, start_date, end_date, access_token, API_FIELDS)
